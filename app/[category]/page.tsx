...
function Page({ params }: { params: Promise<{ category: string }> }) {
  ...
  return (
    <main className='py-8'>
        {isSuccess && categories.some(x=>x.title.toLocaleLowerCase() === category.toLocaleLowerCase()) && 
            publishedArticles.filter(x=>x.type.toLocaleLowerCase()===category.toLocaleLowerCase()).length > 0 ? 
            <div className="containerBloc grid grid-cols-1 lg:grid-cols-3 gap-6 xl:gap-10">
                <div className="flex flex-col gap-10 col-span-1 lg:col-span-2 order-2 lg:order-1">
                    <div className="hidden lg:block">
                        <ArticlePreview version="main" {...publishedArticles.filter(x=>x.type.toLocaleLowerCase()===category.toLocaleLowerCase())[0]}/>
                    </div>
              {/**Articles map */}
                  {publishedArticles.filter(x=>x.type.toLocaleLowerCase()===category.toLocaleLowerCase()).length > 1 ?
              <div className="grid grid-cols-1 gap-7 lg:grid-cols-2">
                  {publishedArticles.filter(x=>x.type.toLocaleLowerCase()===category.toLocaleLowerCase()).slice(0,6).map(article=>(
