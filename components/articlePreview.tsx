...
function ArticlePreview({version = "default", id, type, title, images, className}: PreviewProps) {
  ...
  return (
    <Link href={`/${type}/${id}`} className='py-4 flex flex-col gap-2 font-oswald font-medium'>
    ...
    if(version === "default"){
    return (
        <Link key={id} className="flex flex-col gap-5" href={`/${type}/${title}`}>
    ...
    if(version==="main"){
        return (
            <Link href={`/${type}/${id}`} className={cn('w-full min-h-[calc(100vw*9/16)] lg:min-h-[auto] lg:aspect-video rounded-none lg:rounded-md flex flex-col justify-end relative overflow-hidden', className)}>
