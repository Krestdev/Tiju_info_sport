...
export const usePublishedArticles = () =>{
    ...
    const mainCategories = categories.filter(x=>x.parent !== null && x.articles.length > 0);

    return {
        isSuccess, isLoading, isError, categories, publishedArticles, todayArticles, headline, mainCategories
    }
}